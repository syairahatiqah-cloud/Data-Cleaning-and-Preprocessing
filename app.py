...
# ============================================================
def impute_series_interpolation(series: pd.Series, method: str = "linear", order: int = 2) -> pd.Series:
    """
    Fill NaN gaps using pandas interpolation.
    method='linear' for Linear Interpolation.
    method='polynomial' with order=2/3/etc for Polynomial Interpolation.
    limit_direction='both' allows edge gaps to be filled if possible.
    """
...
def render_interpolation_tab(method_key: str, method_label: str, default_order: int = 2):
    ...
    if run_interp:
        ...
        out_df = build_imputation_output_df(df, time_col, val_col, series, imputed_full, imputed_col)

        n_filled = int(out_df["Imputed_Flag"].sum())
        st.success(f"{method_label} imputation completed. Filled missing values: {n_filled}")

        st.subheader("Preview imputed data")
        st.dataframe(out_df[[time_col, val_col, imputed_col, "Imputed_Flag"]].head(50), use_container_width=True)
        ...
        # --------------------------------------------------------
        st.subheader(f"Original + {method_label} Imputed Segments (Imputed only)")
        ...
        # ✅ SHOW PNG inside app
        st.image(seg_png, caption=f"Original + {method_label} Imputed Segments (PNG via Matplotlib)", use_container_width=True)
        ...
        # --------------------------------------------------------
        st.subheader(f"Monthly Seasonality (Original vs FULL {method_label} Infilled)")
        ...
        # ✅ SHOW PNG inside app
        st.image(season_png, caption=f"Monthly Seasonality: Original vs FULL {method_label} Infilled", use_container_width=True)

        st.download_button(
            f"Download {method_label} monthly seasonality plot (PNG)",
            data=season_png,
            file_name=f"{val_col}_{suffix}_monthly_seasonality_original_vs_full_infilled.png",
            mime="image/png",
